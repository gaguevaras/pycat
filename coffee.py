...
TRIGGERS = {
    ...
    'You are done (.*)\.': lambda mud, matches: mud.mud.log("The task took {}s".format(
        time.time() - (mud.state['task_start_time'] if 'task_start_time' in mud.state else 0))),
...
class Coffee(modular.ModularClient):
    def __init__(self, mud, name):
        ...
        if name == 'grumpy' or (
                False and name == 'grumpier') or name == 'grumpiest' or name == 'dhtnseriao' or name == 'dhtnserioa' or name == 'dhtnseroia':
        ...
        if name == 'zerleha':
            ...
            self.aliases.update({
                'kk( +.+)?': lambda mud, groups: self.stackToLag(
                    'gouge\ntrip\ndirt\nax\nkick\nbodyflip\natemi\nbodytoss\nemote is done with stacking `kk`.',
                    groups[0]),
            })
            # self.triggers.update({
    ...
    def handleGmcp(self, cmd, value):
        ...
        if cmd == 'char.status' and 'pos' in value and 'fatigue' in value and 'maxstats' in self.gmcp['char']:
            if value['pos'] == 'Sleeping' and value['fatigue'] == 0 and self.gmcp['char']['vitals']['moves'] == \
                    self.gmcp['char']['maxstats']['maxmoves']:
                self.log("Rested!")
        if cmd == 'char.vitals' and 'status' in self.gmcp['char'] and 'maxstats' in self.gmcp['char']:
            if self.gmcp['char']['status']['pos'] == 'Sleeping' and value['mana'] == self.gmcp['char']['maxstats'][
                'maxmana']:
                self.onMaxMana()
